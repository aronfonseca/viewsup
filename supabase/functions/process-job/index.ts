    ...
    properties: {
      ...
      profileHealth: {
        ...
        properties: {
          visualConsistency: {
            type: "object",
            description: "Judge this ONLY from the real post thumbnail images included in the message, if any were provided — you are actually looking at them, so answer directly and confidently. STRICTLY LIMIT your observations to: colour palette, typography/text-overlay pattern, framing/composition, lighting, and whether a human face is visible. NEVER infer or state what industry, business type, or niche the person is in based on the filming LOCATION or backdrop (e.g. do not say a modern apartment or office backdrop 'looks like real estate' or any other business — a location is just where they filmed, not what they sell). If no images were provided (see the warning text in the message), stay neutral: hasColorPattern=false, hasFontPattern=false, hostFaceVisible=false, score 40-55, and say in 'insight' that visual analysis could not be verified this run.",
            properties: {
              score: { type: "number" },
...
async function processJob(jobId: string) {
  ...
  try {
    ...
    const seedBenchmarkBlock = seedBenchmarkTable
      ? (isPT
        ? `\n\nBENCHMARK REAL POR NICHO (contas de referência reais, curadas e coletadas por scraping — use como piso de comparação mesmo em nichos com poucas análises orgânicas):\n${seedBenchmarkTable}\n\nApós escolher o "nicho" do perfil, compare a taxa de engajamento real do perfil com a linha correspondente acima, se houver.`
